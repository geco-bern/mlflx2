...
for k in tqdm(range(len(df_sensor_us))):

    train_sites = deepcopy(df_sensor_us)
    conditional_sites  = deepcopy(df_meta_us)
    gpp_sites  =  deepcopy(df_gpp_us)
    
    train_sites = np.delete(train_sites, k)
    test_sites_in = [df_sensor_us[k]]
    conditional_sites =  np.delete(conditional_sites, k)
    conditional_sites_in = [df_meta_us[k]]
    gpp_sites = np.delete(gpp_sites, k)
    gpp_sites_in = [df_gpp_us[k]]
    ...
    x_test_us = [x.values for x in test_sites_in]
    conditional_test_us =  [x.values for x in conditional_sites_in]
    y_test_us = [x.values.reshape(-1, 1) for x in gpp_sites_in]

    x_test_eu = [x.values for x in df_sensor_europe]
    conditional_test_eu =  [x.values for x in df_meta_europe]
    y_test_eu = [x.values.reshape(-1, 1) for x in df_gpp_europe]
    ...
    for epoch in range(args.n_epochs):
        ...
        with torch.no_grad():
                i = 0
                r2 = 0
                for (x, y, conditional) in zip(x_test_us, y_test_us, conditional_test_us):
                    x = torch.FloatTensor(x).to(DEVICE)
                    y = torch.FloatTensor(y).to(DEVICE)
                    c = torch.FloatTensor(conditional).to(DEVICE)
                    y_pred = model(x, c)
...
np.save("train_us_bias_europe.npy",bias_europe)
np.save("train_us_bias_us.npy",bias_us)
